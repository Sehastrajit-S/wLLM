"""Native Llama-family forward pass, written directly on top of torch --
...
This is the base architecture every model this codebase supports actually
runs on (RMSNorm, RoPE, grouped-query attention, SwiGLU MLP): Qwen2 is a
`LlamaForCausalLM` with a config-driven qkv bias (see qwen2.py, which
subclasses this module the same way real vLLM's Qwen2Model subclasses its
LlamaModel), and every checkpoint listed in ARCHITECTURE_REGISTRY that
publishes an HF config in this same shape (Yi, XVERSE, Baichuan2, InternLM2,
Granite, Orion, ...) loads through this exact class, unmodified.
...
class LlamaAttention(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        # cfg.attention_bias is what lets this one class serve both Llama
        # (no bias) and Qwen2 (bias) -- see ModelConfig.attention_bias and
        # qwen2.py's docstring for why that's the only structural difference
        # between the two. o_proj has no bias in either.
...
class LlamaModel(nn.Module):
    # Overridden by GemmaModel (to hidden_size**0.5): real Gemma scales
    # embeddings right after lookup, every place embed_tokens is read. Rather
    # than duplicate that multiply across every call site below (forward,
    # prefill, chunked prefill, decode, graphable decode), each one calls
    # `embed()` instead of `embed_tokens()` directly, so this is the one
    # place a subclass needs to change.
    ...
    def __init__(self, cfg: ModelConfig):
        ...
        # A buffer, not a plain attribute, so `.to(device=..., dtype=...)`
        # (load_native casts the whole model right after construction) moves
        # and casts it along with everything else -- by the time any forward
        # pass runs, this is already the right dtype/device, so embed()'s
        # multiply needs no `.to()`/`torch.tensor()` call of its own. That
        # matters beyond style: allocating a fresh tensor inside a CUDA-graph
        # capture (as embed() used to, casting self.embed_scale on every
        # call) raises `cudaErrorStreamCaptureUnsupported` -- capture only
        # allows replaying operations on already-resident buffers.
        self.register_buffer("_embed_scale_tensor", torch.tensor(self.embed_scale), persistent=False)

    def embed(self, input_ids: torch.Tensor) -> torch.Tensor:
        x = self.embed_tokens(input_ids)
        if self.embed_scale != 1.0:
            x = x * self._embed_scale_tensor
        return x
...
class LlamaForCausalLM(nn.Module):
    # Overridden by subclasses (e.g. qwen2.py's Qwen2ForCausalLM sets this to
    # Qwen2Model) so `self.model` is constructed as the subclass's own model
    # class without each subclass needing to repeat this __init__ verbatim --
    # the same shape as real vLLM's per-architecture ForCausalLM classes.
    _model_cls = LlamaModel

    def __init__(self, cfg: ModelConfig):
        super().__init__()
        self.cfg = cfg
        self.model = self._model_cls(cfg)
    ...
    def decode_step_batch(self, cache, seq_ids: list, token_ids: torch.Tensor, lora_ids=None, lora_registry=None) -> torch.Tensor:
        ...
        x = self.model.embed(token_ids)  # (num_seqs, 1, hidden)
    ...
    def decode_step_graphable(self, cache, token_ids, position_ids, block_table, context_len, write_idx):
        ...
        """
        x = self.model.embed(token_ids)  # (batch, 1, hidden)
        cos, sin = self.model.rotary_emb(position_ids)
...
@torch.inference_mode()
def generate_greedy_naive(model: LlamaForCausalLM, input_ids: torch.Tensor, max_new_tokens: int) -> torch.Tensor:
...
def load_native(model_id: str, device: str = "cuda", dtype: torch.dtype = torch.bfloat16) -> LlamaForCausalLM:
    """Generic HF-safetensors loader for any architecture registered in
    wllm.models.registry: reads config.json's `architectures` field, looks up
    the matching model class (real vLLM does the same dispatch, at far
    larger scale -- see registry.py's docstring), then constructs and loads
    that class normally. Every architecture registered today resolves to
    either LlamaForCausalLM or Qwen2ForCausalLM, since that's genuinely what
    each of their checkpoints is under the hood.
    """
    from wllm.models.registry import resolve_model_class
    ...
    with open(config_path, "r", encoding="utf-8") as f:
        raw = json.load(f)
    model_cls = resolve_model_class(raw.get("architectures") or [])
